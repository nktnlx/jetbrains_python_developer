...
def next_move(inp, user_choice, move):
    lst = list(inp)
    if move % 2 == 0:
        lst[user_choice] = 'X'
    else:
...
def game_states(state):
    ...
    line_1 = state[1][2] + state[1][4] + state[1][6] # creating winning lines
    line_2 = state[2][2] + state[2][4] + state[2][6] # creating winning lines
    line_3 = state[3][2] + state[3][4] + state[3][6] # creating winning lines
    ...
    # 'X' winning options list
    x_win = [dia_1.count('X') == 3,
             dia_2.count('X') == 3,
             col_1.count('X') == 3,
             col_2.count('X') == 3,
             col_3.count('X') == 3,
    ...
    # 'O' winning options list
    o_win = [dia_1.count('O') == 3,
             dia_2.count('O') == 3,
             col_1.count('O') == 3,
             col_2.count('O') == 3,
             col_3.count('O') == 3,
    ...
    # counting empty fields left
    underscore_count = 0
    for el in state[1:4]:
        underscore_count += el.count('_')
...
def main(user_input):
    game = field(user_input)
    render(game)
    status = game_states(game)
    move = 0  # helps to alternate X and O moves (one after another)
    while status == 'Game in progress':
        new_cells = empty(user_input)
        user_ind = enter_coord(new_cells)
        next_field = next_move(user_input, user_ind, move)
        move += 1
        game = field(next_field)
        render(game)
        user_input = next_field
        status = game_states(game)

    print(game_states(game))
