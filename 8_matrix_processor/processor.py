...
def invert_matrix(matrix):
    ...
    matrix_copy = matrix[:]
    # creating an identity(unity) matrix of the same size as an input matrix
    identity_matrix = [[0 if i != k else 1 for i in range(len(matrix[0]))] for k in range(len(matrix))]
    identity_matrix_copy = identity_matrix[:]

    # Perform row operations
    row_ind = list(range(n))  # to allow row referencing
    for fd in range(n):  # fd stands for focus diagonal
        fd_scaler = 1.0 / matrix_copy[fd][fd]
        # FIRST: scale fd row with fd inverse.
        for j in range(n):  # Use j to indicate column looping.
            matrix_copy[fd][j] *= fd_scaler
            identity_matrix_copy[fd][j] *= fd_scaler
        # SECOND: operate on all rows except fd row as follows:
        for i in row_ind[0:fd] + row_ind[fd+1:]:
            cr_scaler = matrix_copy[i][fd]  # cr stands for "current row".
            for j in range(n):
                # cr - cr_scaler * fdRow, but one element at a time.
                matrix_copy[i][j] = matrix_copy[i][j] - cr_scaler * matrix_copy[fd][j]
                identity_matrix_copy[i][j] = identity_matrix_copy[i][j] - cr_scaler * identity_matrix_copy[fd][j]

    return identity_matrix_copy
